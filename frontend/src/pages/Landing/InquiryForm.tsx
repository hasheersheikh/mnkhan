...
const InquiryForm: React.FC<InquiryFormProps> = ({
  ...
  const formContent = (
    <div
      className={`max-w-4xl mx-auto relative z-10 ${isEmbedded ? "" : "py-12 md:py-24"}`}
    >
      {!isEmbedded && (
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-serif mb-4 italic">
            Start a Conversation
          </h2>
          <p className="text-white/60 text-lg">
            Send us an inquiry and our legal experts will get back to you.
          </p>
        </div>
      )}
      ...
      {status === "success" ? (
        <div
          className={`backdrop-blur-md border border-mnkhan-orange p-12 text-center rounded-sm animate-in fade-in zoom-in duration-500 ${theme === "dark" ? "bg-white/10" : "bg-mnkhan-orange/5"}`}
        >
          <div className="w-16 h-16 bg-mnkhan-orange rounded-full flex items-center justify-center mx-auto mb-6">
            <span className="text-2xl font-bold text-white">✓</span>
          </div>
          <h3
            className={`text-3xl font-serif mb-4 ${theme === "dark" ? "text-white" : "text-mnkhan-charcoal"}`}
          >
            Inquiry Received
          </h3>
          <p
            className={`leading-relaxed ${theme === "dark" ? "text-white/70" : "text-mnkhan-text-muted"}`}
          >
            Thank you for reaching out to MN KHAN. A member of our team will
            contact you shortly regarding your request.
          </p>
          <button
        ...
        <form
          ...
          className={`grid grid-cols-1 md:grid-cols-2 gap-8 backdrop-blur-xl p-6 md:p-10 lg:p-16 border rounded-sm ${theme === "dark" ? "bg-white/5 border-white/10" : "bg-gray-50/50 border-mnkhan-gray-border"}`}
        >
          <div className="space-y-2">
            <label
              className={`text-[10px] font-bold uppercase tracking-[0.2em] ${theme === "dark" ? "text-white/40" : "text-mnkhan-text-muted"}`}
            >
              Full Name
            </label>
            <input
          ...
          <div className="space-y-2">
            ...
            <div
              className={`flex items-center gap-2 border-b focus-within:border-mnkhan-orange transition-colors ${theme === "dark" ? "border-white/20" : "border-mnkhan-gray-border"}`}
            >
              <span
                className={`font-bold py-3 ${theme === "dark" ? "text-white" : "text-mnkhan-charcoal"}`}
              >
                +91
              </span>
              <input
          ...
          <div className="col-span-full space-y-2">
            ...
            >
              <option
                value=""
                disabled
                className={theme === "dark" ? "text-white/20" : "text-gray-400"}
              >
                How can we help you?
              </option>
              {services.map((s) => (
                <option
                  ...
                  className={
                    theme === "dark"
                      ? "bg-mnkhan-charcoal text-white"
                      : "bg-white text-mnkhan-charcoal"
                  }
                >
                  {s.name}
                </option>
  ...
  return (
    <section
      id="contact"
      className="px-4 md:px-8 bg-mnkhan-charcoal text-white relative overflow-hidden"
    >
      {/* Decorative Orbs */}
