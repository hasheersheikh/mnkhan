...
const ManageClients: React.FC = () => {
  ...
  const handleStatusUpdate = async (id: string, newStatus: string) => {
    ...
    if (newStatus === "rejected") {
      rejectionReason = prompt(
        "Please enter the reason for rejection (this will be emailed to the client):",
      );
    ...
    setUpdating(id);
    try {
      const res = await updateUserStatus(id, newStatus, rejectionReason);
      if (res.data.success) {
        setUsers(
          users.map((u) =>
            u._id === id ? { ...u, status: newStatus as any } : u,
          ),
        );
      }
    } catch (err: any) {
  ...
  const handleResetPassword = async (id: string) => {
    ...
    if (
      !confirm("Are you sure you want to force reset this client's password?")
    )
      return;
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      ...
      case "pending":
        return (
          <span className="bg-orange-100 text-orange-700 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider">
            Pending Approval
          </span>
        );
      case "deactivated":
      ...
      <div className="flex justify-between items-end mb-10">
        <div>
          ...
          <p className="text-sm text-mnkhan-text-muted">
            Manage client access, approvals, and security status.
          </p>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-mnkhan-charcoal text-white">
              ...
              <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-right">
                Actions
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-mnkhan-gray-border">
            {users.length === 0 ? (
              <tr>
                <td
                  colSpan={5}
                  className="px-6 py-12 text-center text-mnkhan-text-muted italic"
                >
              ...
              users.map((user) => (
                ...
                >
                  <td className="px-6 py-5">
                    <div className="font-bold text-mnkhan-charcoal">
                      {user.name}
                    </div>
                    <div className="text-xs text-mnkhan-text-muted">
                      {user.email}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-5">{statusBadge(user.status)}</td>
                      ...
                      {user.status === "pending" && (
                        <>
                          ...
                          <button
                            ...
                            className="bg-red-600 text-white px-4 py-2 text-[10px] font-bold uppercase tracking-widest hover:bg-red-700 transition-colors disabled:opacity-50"
                          >
                            {updating === user._id ? "..." : "Reject"}
                          </button>
                        </>
                      )}
                      {user.status === "active" && (
                        <button
                          disabled={!!updating}
                          onClick={() =>
                            handleStatusUpdate(user._id, "deactivated")
                          }
