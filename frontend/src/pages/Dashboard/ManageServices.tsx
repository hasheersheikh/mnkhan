...
const ManageServices: React.FC = () => {
  ...
  const handleDeleteService = async (id: string) => {
    if (!window.confirm('Delete this legal service? This action cannot be undone.')) return;
    try {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex justify-between items-end border-b border-mnkhan-gray-border pb-6">
        <div>
          <h2 className="text-3xl font-serif italic text-mnkhan-charcoal mb-2">Legal Product Catalog</h2>
          <p className="text-mnkhan-text-muted text-[10px] font-bold uppercase tracking-[0.3em]">Service Portfolio Management</p>
        </div>
        <div className="flex items-center gap-2 px-4 py-2 bg-mnkhan-orange/10 text-mnkhan-orange rounded-sm border border-mnkhan-orange/20">
          <Layers size={14} />
          <span className="text-[10px] font-bold uppercase tracking-widest">{services.length} Active Services</span>
        </div>
      </div>
        ...
        <div className="col-span-12 lg:col-span-4 self-start sticky top-24">
          <div className="bg-white p-8 rounded-sm shadow-sm border border-mnkhan-gray-border relative overflow-hidden">
            ...
            <form onSubmit={handleCreateService} className="space-y-6 relative z-10">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-mnkhan-charcoal">Short Prospectus</label>
                <textarea 
                  required
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-mnkhan-charcoal">Service Provided (Full Details)</label>
                <textarea 
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-mnkhan-charcoal">Acceptance Criteria</label>
                <textarea 
              ...
              <div className="space-y-4 pt-4 border-t border-mnkhan-gray-border">
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                  {form.defaultSteps.map((s, i) => (
                    <div key={i} className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest bg-mnkhan-gray-light/20 p-2.5 rounded border border-mnkhan-gray-border/50 group animate-in slide-in-from-left-2">
                      <span className="flex items-center gap-2">
                        <span className="text-mnkhan-orange">{i + 1}.</span> {s}
                      </span>
                      <button type="button" onClick={() => removeStep(i)} className="text-red-400 hover:text-red-600 transition-colors opacity-0 group-hover:opacity-100">
                        <X size={14} />
                      </button>
                    </div>
                  ))}
                  {form.defaultSteps.length === 0 && (
                    <p className="text-[9px] text-mnkhan-text-muted italic opacity-60 text-center py-2">No milestones defined yet.</p>
                  )}
                </div>
              </div>

              <div className="flex flex-col gap-3 pt-6">
                <button type="submit" className="w-full bg-mnkhan-charcoal text-white py-4 font-bold uppercase tracking-widest text-[10px] hover:bg-mnkhan-orange transition-all shadow-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {loading ? (
              <div className="col-span-full flex flex-col items-center justify-center py-40 opacity-30">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest italic tracking-[0.3em]">Auditing Catalog...</p>
              </div>
            ) : services.length > 0 ? (
                ...
                >
                  ...
                  <div className="mb-6">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="text-xl font-serif italic text-mnkhan-charcoal group-hover:text-mnkhan-orange transition-colors">{s.name}</h4>
                    ...
                    <p className="text-xs text-mnkhan-text-muted leading-relaxed line-clamp-2 italic">"{s.description}"</p>
                  </div>

                  <div className="space-y-4 mb-8">
                    <div className="flex items-center gap-2">
                      <div className="h-0.5 flex-1 bg-mnkhan-gray-border" />
                      <span className="text-[9px] font-bold uppercase tracking-widest text-mnkhan-text-muted">Lifecycle Milestones</span>
                      <div className="h-0.5 flex-1 bg-mnkhan-gray-border" />
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {s.defaultSteps?.length > 0 ? (
                        s.defaultSteps.map((step: string, si: number) => (
                          <span key={si} className="text-[8px] bg-mnkhan-gray-light/30 border border-mnkhan-gray-border/50 text-mnkhan-charcoal px-2 py-1 rounded-sm font-bold uppercase tracking-tighter">
                            {step}
                          </span>
                        ))
                      ) : (
                        <span className="text-[8px] text-mnkhan-text-muted italic opacity-50">No default workflow defined</span>
                      )}
                    </div>
                  </div>
              ...
              <div className="col-span-full flex flex-col items-center justify-center py-32 opacity-20">
                ...
                <p className="text-xs font-bold uppercase tracking-widest italic tracking-[0.2em]">Service Catalog is Empty</p>
              </div>
            )}
          </div>
        </div>
