...
const Header: React.FC<{ onLogout: () => void; onMenuClick: () => void }> = ({
  onLogout,
  onMenuClick,
}) => {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      const [
        ...
        tasksApi.getTasks().catch(() => ({ data: { tasks: [] } })),
        servicesApi.getServices().catch(() => ({ data: { services: [] } })),
        isAdmin
        ...
        isAdmin
          ...
          : documentsApi
              ...
              .catch(() => ({ data: { documents: [] } })),
        isAdmin
          ? vouchersApi.getVouchers().catch(() => ({ data: { vouchers: [] } }))
          : Promise.resolve({ data: { success: true, vouchers: [] } }),
        blogsApi.getBlogs().catch(() => ({ data: { blogs: [] } })),
      ]);

      const q = query.toLowerCase();

      const filteredTasks = (tasksRes.data.tasks || [])
        .filter(
          (t: any) =>
      ...
      const filteredServices = (servicesRes.data.services || [])
        .filter(
          (s: any) =>
      ...
      const filteredPeople = (usersRes.data.users || [])
        .filter(
          (u: any) =>
      ...
      const filteredInquiries = (inquiriesRes.data.inquiries || [])
        .filter(
          (i: any) =>
      ...
      const filteredDocuments = (documentsRes.data.documents || [])
        .filter(
          (d: any) =>
      ...
      const filteredVouchers = (vouchersRes.data.vouchers || [])
        .filter(
          (v: any) =>
  ...
  return (
    <header className="fixed top-0 left-0 md:left-[260px] right-0 z-[90] flex h-[70px] items-center justify-between bg-white px-4 md:px-8 border-b border-mnkhan-gray-border">
      <div className="flex items-center gap-3 md:gap-4 flex-1">
        <button
          className="md:hidden text-mnkhan-charcoal p-1 hover:bg-mnkhan-gray-light rounded"
          onClick={onMenuClick}
        >
          <Menu size={20} />
        </button>
          ...
          <div className="relative group">
            ...
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1 pointer-events-none">
              {isSearching ? (
                <Loader2
                  size={12}
                  className="animate-spin text-mnkhan-orange"
                />
              ) : !searchQuery ? (
            ...
            <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-mnkhan-gray-border shadow-2xl rounded-sm overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                ...
                !isSearching ? (
                  <div className="p-8 text-center">
                    <p className="text-xs text-mnkhan-text-muted italic uppercase tracking-widest">
                  ...
                  <>
                    {searchResults.tasks.length > 0 && (
                      <div className="p-2 border-b border-mnkhan-gray-border last:border-0">
                        <p className="px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest text-mnkhan-orange bg-mnkhan-orange/5 rounded-sm mb-1">
                          Active Matters
                        </p>
                        {searchResults.tasks.map((task) => (
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              <p className="text-xs font-bold text-mnkhan-charcoal truncate">
                                {task.title}
                              </p>
                    ...
                    {searchResults.services.length > 0 && (
                      <div className="p-2 border-b border-mnkhan-gray-border last:border-0">
                        <p className="px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest text-mnkhan-charcoal bg-mnkhan-gray-light/50 rounded-sm mb-1">
                          Legal Services
                        </p>
                    ...
                    {isAdmin && searchResults.people.length > 0 && (
                      <div className="p-2 border-b border-mnkhan-gray-border last:border-0">
                        <p className="px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest text-mnkhan-text-muted bg-mnkhan-gray-bg rounded-sm mb-1">
                          Clients & Users
                        </p>
                    ...
                    {searchResults.inquiries.length > 0 && (
                      <div className="p-2 border-b border-mnkhan-gray-border last:border-0">
                        <p className="px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest text-[#8b5cf6] bg-[#f5f3ff] rounded-sm mb-1">
                          Inquiries
                        </p>
                    ...
                    {searchResults.documents.length > 0 && (
                      <div className="p-2 border-b border-mnkhan-gray-border last:border-0">
                        <p className="px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest text-[#0ea5e9] bg-[#f0f9ff] rounded-sm mb-1">
                          Documents
                        </p>
                    ...
                    {searchResults.vouchers.length > 0 && (
                      <div className="p-2 border-b border-mnkhan-gray-border last:border-0">
                        <p className="px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest text-[#10b981] bg-[#ecfdf5] rounded-sm mb-1">
                          Vouchers
                        </p>
                        {searchResults.vouchers.map((voucher) => (
                          <button
                            key={voucher._id}
                            onClick={() => navTo("/portal/admin-vouchers")}
                            className="w-full text-left px-3 py-2 hover:bg-mnkhan-gray-light/50 rounded transition-colors flex items-center gap-3"
                          >
                    ...
                    {searchResults.blogs.length > 0 && (
                      <div className="p-2 border-b border-mnkhan-gray-border last:border-0">
                        <p className="px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest text-[#f43f5e] bg-[#fff1f2] rounded-sm mb-1">
                          Insights & Knowledge
                        </p>
                        {searchResults.blogs.map((blog) => (
                          <button
                            ...
                            className="w-full text-left px-3 py-2 hover:bg-mnkhan-gray-light/50 rounded transition-colors flex items-center gap-3"
                          >
                            <Globe size={14} className="text-[#f43f5e]" />
                            <div className="min-w-0">
                              <p className="text-xs font-bold text-mnkhan-charcoal truncate">
                                {blog.title}
                              </p>
              ...
              {searchQuery.length >= 2 && !isSearching && (
                <div className="bg-mnkhan-gray-bg py-2 px-4 border-t border-mnkhan-gray-border">
                  <p className="text-[10px] text-mnkhan-text-muted font-bold uppercase tracking-tighter">
                    Press ESC to close • Showing top matches
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {showDropdown && (
            <div className="absolute right-0 mt-3 w-48 bg-white border border-mnkhan-gray-border shadow-xl rounded py-2 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <button
                onClick={() => {
                  window.location.href = "/portal/account-security";
                  setShowDropdown(false);
                }}
