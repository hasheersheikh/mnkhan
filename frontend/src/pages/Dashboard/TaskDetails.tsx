  ...
  if (!task) {
    return (
      <div className="bg-red-50 p-12 text-center border border-red-100 rounded-sm">
        <p className="text-red-700 font-serif italic text-lg mb-2">
          Matter Record Not Found
        </p>
        <p className="text-[10px] font-bold uppercase tracking-widest text-red-400">
          The requested procedural file does not exist or access is restricted.
        </p>
        <button
          onClick={() => navigate(-1)}
          className="mt-8 text-[10px] font-bold uppercase tracking-widest text-mnkhan-charcoal underline underline-offset-4"
        >
          Return to Registry
        </button>
      </div>
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-[1600px] mx-auto pb-20">
        ...
        <div className="flex items-start gap-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-mnkhan-orange">
                Authorized Personnel Access Only
              </span>
            </div>
            <h2 className="text-4xl font-serif italic text-mnkhan-charcoal leading-tight">
              {task.title}
            </h2>
            <div className="flex items-center gap-4 mt-3">
              <p className="text-[10px] font-bold uppercase tracking-widest text-mnkhan-text-muted">
                Status:{" "}
                <span className="text-mnkhan-charcoal font-black">
                  {task.status}
                </span>
              </p>
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-mnkhan-text-muted">
                Matter ID:{" "}
                <span className="text-mnkhan-charcoal opacity-60">
        ...
        <div className="flex flex-col items-end gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="text-right hidden sm:block">
              <p className="text-[9px] font-bold uppercase tracking-widest text-mnkhan-text-muted mb-1">
                Finalized on
              </p>
              <p className="text-xs font-serif italic text-mnkhan-charcoal">
      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        {activeTab === "overview" ? (
          <MatterOverview
            ...
            isAdmin={isAdmin}
            isStaff={isStaff}
            assignedStaffId={task.assignedStaffId?._id}
          />
        ) : (
          <MatterDocuments
            taskId={task._id}
      ...
      <StaffAssignmentModal
        ...
        onAssign={async (staffId) => {
          const res = await tasksApi.updateTask(task._id, {
