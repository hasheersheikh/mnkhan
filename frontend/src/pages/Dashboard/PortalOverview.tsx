...
const PortalOverview: React.FC<OverviewProps> = ({
  userName,
  isAdmin,
  isStaff,
}) => {
    ...
    try {
      ...
      if (isAdmin) {
        ...
        if (inqRes.data.success)
          setInquiries(inqRes.data.inquiries.slice(0, 5));
      }
    } catch (err) {
      ...
      return [
        {
          label: "Firm Matters",
          value: tasks.length,
          color: "text-mnkhan-orange",
        },
        {
          label: "Active Inquiries",
          value: inquiries.length,
          color: "text-mnkhan-charcoal",
        },
        {
  ...
  const getSubtext = () => {
    if (isAdmin)
      return "Systems are online. Administrative overview of firm matters.";
    if (isStaff)
      return "Focusing on your assigned portfolios and professional development.";
    ...
    <>
      <div className="mb-8 animate-in fade-in slide-in-from-top-4 duration-700">
        <h1 className="text-4xl font-serif font-normal text-mnkhan-charcoal uppercase tracking-tight">
          {isStaff ? "Operations" : isAdmin ? "Registry" : "Client"} Overview.{" "}
          <span className="text-mnkhan-orange italic">{userName}</span>
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100">
        {statCards.map((stat, i) => (
          <div
            key={i}
            className="bg-white p-10 rounded-sm border border-mnkhan-gray-border flex flex-col group hover:border-mnkhan-orange transition-all duration-500 shadow-sm hover:shadow-xl"
          >
            <p className="text-[9px] font-bold uppercase tracking-[0.3em] text-mnkhan-text-muted mb-4 group-hover:text-mnkhan-orange transition-colors">
              {stat.label}
            </p>
            <p className={`text-4xl font-serif italic ${stat.color}`}>
              {stat.value}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-200">
        ...
        {isAdmin ? (
          ...
          <div className="col-span-12 lg:col-span-4 bg-white p-6 md:p-8 border border-mnkhan-gray-border rounded-sm">
            <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-mnkhan-charcoal mb-6 border-b border-mnkhan-gray-border pb-4">
            ...
            <ul className="space-y-4">
              {[
                ...
                <li
                  key={idx}
                  className="text-xs text-mnkhan-text-muted flex gap-3 italic"
                >
                  <span className="text-mnkhan-orange font-bold font-mono">
                    {idx + 1}.
                  </span>
                  {rule}
                </li>
