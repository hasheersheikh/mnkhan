...
const ManageTasks: React.FC = () => {
  ...
  const handleUpdateAssignment = async (staffId: string) => {
    ...
    try {
      const res = await tasksApi.updateTask(assignmentModal.taskId, {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)] animate-in fade-in duration-500">
      {/* Header Bar */}
      <div className="flex justify-between items-center flex-wrap gap-4 mb-6 bg-white p-4 border border-mnkhan-gray-border rounded-sm shadow-sm">
        <div className="flex items-center gap-3">
          <div className="bg-mnkhan-charcoal p-2 rounded-sm text-white">
            <ClipboardList size={20} />
          </div>
          <div>
            <h2 className="text-xl font-bold text-mnkhan-charcoal">
              Global Matter Management
            </h2>
            <p className="text-[10px] text-mnkhan-text-muted uppercase tracking-widest font-bold">
              Administrate Workflow & Communications
            </p>
          </div>
        </div>
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 font-bold uppercase tracking-widest text-[10px] transition-all rounded-sm ${
              isAssigning
                ? "bg-mnkhan-orange text-white shadow-lg"
      ...
      <div className="flex flex-col md:flex-row flex-1 gap-4 md:gap-6 overflow-hidden">
        ...
        <div className="w-full md:w-80 h-[200px] md:h-auto flex-shrink-0 flex flex-col bg-white border border-mnkhan-gray-border rounded-sm shadow-sm overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            <button
              ...
              className={`w-full flex items-center gap-3 p-4 text-left border-b border-mnkhan-gray-light hover:bg-mnkhan-gray-light/10 transition-colors ${
                selectedClientId === null
                  ? "bg-mnkhan-orange/5 border-l-4 border-l-mnkhan-orange pl-3"
            ...
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                  selectedClientId === null
                    ...
                    : "bg-mnkhan-gray-border text-mnkhan-charcoal"
                }`}
              >
                <Users size={14} />
              </div>
              <div className="flex-1 min-w-0">
                <p
                  className={`text-xs font-bold truncate ${selectedClientId === null ? "text-mnkhan-orange" : "text-mnkhan-charcoal"}`}
                >
                  All Clients
                </p>
                <p className="text-[10px] text-mnkhan-text-muted truncate">
                  Consolidated View
                </p>
              </div>
            </button>
            {filteredClients.map((client) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-xs font-bold truncate ${selectedClientId === client._id ? "text-mnkhan-orange" : "text-mnkhan-charcoal"}`}
                  >
                    {client.name}
                  </p>
                  <p className="text-[10px] text-mnkhan-text-muted truncate">
          ...
          {isAssigning ? (
            <div className="flex-1 bg-white border border-mnkhan-gray-border rounded-sm shadow-sm overflow-y-auto custom-scrollbar p-8">
              <div className="max-w-2xl mx-auto w-full animate-in slide-in-from-bottom-4 duration-500">
                <h3 className="text-2xl font-serif italic mb-2 text-mnkhan-charcoal">
                  Assign New Legal Matter
                </h3>
                <p className="text-xs text-mnkhan-text-muted mb-8 border-b border-mnkhan-gray-border pb-4">
                  Define a new case or task for a client account.
                </p>
                ...
                <form onSubmit={handleCreateTask} className="space-y-6">
                        ...
                        onChange={(e) => {
                          ...
                          if (service) {
                            setTaskForm({
                              ...
                              steps: (service.defaultSteps || []).map(
                                (s: string) => ({ title: s, completed: false }),
                              ),
                            });
                          }
                        }}
                  ...
                  <div className="space-y-4 bg-mnkhan-gray-light/5 p-6 rounded-sm border border-mnkhan-gray-border border-dashed">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-[9px] text-mnkhan-orange font-bold uppercase tracking-tighter">
                        Auto-calculates progress based on completion
                      </span>
                    </div>
                    ...
                    <div className="space-y-2">
                      {taskForm.steps.map((step, idx) => (
                        <div
                          key={idx}
                          className="flex justify-between items-center text-xs bg-white p-3 rounded border border-mnkhan-gray-border group"
                        >
                          <span className="font-medium text-mnkhan-charcoal">
                            {idx + 1}. {step.title}
                          </span>
                          <button
                            type="button"
                      ...
                      {!taskForm.steps.length && (
                        <div className="py-4 text-center">
                          <p className="text-[10px] text-mnkhan-text-muted italic uppercase tracking-widest">
              ...
              <div className="p-6 bg-white border border-mnkhan-gray-border rounded-sm shadow-sm flex justify-between items-center mb-6">
                <div>
                  <h3 className="text-lg font-bold text-mnkhan-charcoal">
                    {selectedClientId
                      ? selectedClient?.name
                      : "Consolidated Matter Feed"}
                  </h3>
                  <p className="text-xs text-mnkhan-text-muted">
                    {selectedClientId
                      ? `${selectedClient?.email} • ${tasks.length} Active Matters`
                      : `${tasks.length} Matters across all clients`}
                  </p>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-2 custom-scrollbar">
                {loading ? (
                  <div className="flex flex-col items-center justify-center py-20 opacity-40">
                    <Clock
                      className="text-mnkhan-orange mb-4 animate-spin"
                      size={40}
                    />
                    <p className="text-[10px] font-bold uppercase tracking-widest">
                      Indexing Legal Matters...
                    </p>
                  </div>
                ) : tasks.length > 0 ? (
                      ...
                      >
                        <div className="p-6 flex-1">
                          <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="flex flex-col items-end gap-2">
                              <span
                                className={`text-[8px] font-bold uppercase tracking-widest px-2 py-1 rounded-sm text-white ${
                                  t.status === "completed"
                                    ? "bg-green-600"
                          ...
                          <h4 className="font-bold text-mnkhan-charcoal text-sm mb-2 line-clamp-1 group-hover:text-mnkhan-orange transition-colors">
                            {t.title}
                          </h4>
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-[10px] font-bold text-mnkhan-charcoal">
                                {t.progress}%
                              </span>
                            </div>
                        ...
                        <div className="px-6 py-4 bg-mnkhan-gray-light/10 border-t border-mnkhan-gray-border flex justify-between items-center">
                          <div className="flex items-center gap-3">
                            <Clock
                              size={12}
                              className="text-mnkhan-text-muted"
                            />
                  ...
                  <div className="flex flex-col items-center justify-center py-24 text-center">
                    ...
                    <h4 className="text-lg font-bold text-mnkhan-charcoal mb-2">
                      No Active Matters Found
                    </h4>
                    <p className="text-xs text-mnkhan-text-muted max-w-xs mx-auto">
                      No matters found matching your selection. Start by
                      assigning a new legal matter to a client.
                    </p>
                    <button
