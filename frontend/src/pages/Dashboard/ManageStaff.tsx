...
const ManageStaff: React.FC = () => {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        setMessage({ type: 'success', text: `Password for ${resetModal.name} has been reset.` });
        setResetModal(null);
        setNewPassword('');
        setTimeout(() => setMessage(null), 5000);
      }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex justify-between items-end border-b border-mnkhan-gray-border pb-6">
        <div>
          <h2 className="text-3xl font-serif italic text-mnkhan-charcoal mb-2">Firm Staff Directory</h2>
          <p className="text-mnkhan-text-muted text-[10px] font-bold uppercase tracking-[0.3em]">Access Management & Internal Team</p>
        </div>
        <button 
          onClick={() => setIsAdding(!isAdding)}
          className={`px-6 py-2.5 font-bold uppercase tracking-widest text-[10px] transition-all rounded-sm shadow-lg ${
            isAdding ? 'bg-mnkhan-orange text-white' : 'bg-mnkhan-charcoal text-white hover:bg-mnkhan-orange'
          }`}
        >
          {isAdding ? 'Cancel Onboarding' : <><UserPlus size={14} className="inline mr-2" /> Onboard Staff Member</>}
        </button>
      </div>

      {message && (
        <div className={`p-4 rounded-sm border ${message.type === 'success' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'} animate-in slide-in-from-top-2 duration-300 text-xs font-bold uppercase tracking-widest`}>
          {message.text}
        </div>
      )}
        ...
        {isAdding && (
          <div className="col-span-12 lg:col-span-4 animate-in slide-in-from-left-4 duration-500">
            <div className="bg-white p-8 rounded-sm shadow-sm border border-mnkhan-gray-border relative overflow-hidden">
              ...
              <h3 className="text-xl font-serif italic text-mnkhan-charcoal mb-6 relative z-10">New Staff Credentials</h3>
              
              <form onSubmit={handleCreateStaff} className="space-y-6 relative z-10">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-mnkhan-charcoal">Full Legal Name</label>
                  <input 
                    type="text" required
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-mnkhan-charcoal">Corporate Email</label>
                  <input 
                    type="email" required
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-mnkhan-charcoal">Temporary Access Key (Password)</label>
                  <input 
                ...
                <button type="submit" className="w-full bg-mnkhan-charcoal text-white py-4 font-bold uppercase tracking-widest text-[10px] hover:bg-mnkhan-orange transition-all shadow-md mt-4">
                  Provision Account
                </button>
              </form>
            </div>
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan={4} className="p-12 text-center opacity-40">
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest italic">Synchronizing Team Data...</span>
                      </td>
                    </tr>
                  ) : filteredStaff.length > 0 ? (
                    filteredStaff.map((s) => (
                      <tr key={s._id} className="border-b border-mnkhan-gray-light hover:bg-mnkhan-gray-light/10 transition-colors group">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-bold text-mnkhan-charcoal">{s.name}</span>
                          </div>
                        </td>
                        <td className="p-4 text-xs text-mnkhan-text-muted">{s.email}</td>
                        <td className="p-4">
      ...
      {resetModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-mnkhan-charcoal/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md p-8 rounded-sm shadow-2xl border border-mnkhan-gray-border animate-in zoom-in-95 duration-300">
            <h3 className="text-2xl font-serif italic text-mnkhan-charcoal mb-2">Reset Portal Access</h3>
            <p className="text-xs text-mnkhan-text-muted mb-8 italic">Generating new credentials for {resetModal.name}</p>
            
            <div className="space-y-6">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-mnkhan-charcoal">Master Access Key</label>
                <input 
                  type="password" 
                  autoFocus
